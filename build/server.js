...
server.listen(
  {
    port: Number((_a = process.env.PORT) != null ? _a : 4300),
    host: (_b = process.env.SERVER_HOSTNAME) != null ? _b : "0.0.0.0"
  },
  (err, address) => {
    if (err) {
...
server.ready(async (err) => {
  ...
  logger.info(`Server listening on port ${Number((_a2 = process.env.PORT) != null ? _a2 : 4300)}`);
});
